...
var tcresearch = {
    ...
    html: {
        searchResult:
            ...
            '</div>',
        aspectsList: '<ul class="aspect-list">' + '</ul>',
        searchAspect:
            '<li class="aspect">' +
            '<img />' +
        ...
        searchInfo:
            ...
            '</div>',
        usedAspect:
            '<li class="used-aspect">' + '<img />' + '<span></span>' + '</li>',
        toggleAddon:
            '<label class="btn btn-default"><input type="checkbox" class="addon_toggle" /><span></span></label>'
    },

    // Add an unidirectional connection between two aspects in the graph
    addConnection: function (from, to) {
        if (!(from in this.graph)) this.graph[from] = [];
    ...
    toggle: function (aspect) {
        if ($(aspect).hasClass('unavail')) this.enableAspect(aspect);
        else this.disableAspect(aspect);
    },
    ...
    run: function () {
        ...
        path.forEach(function (aspect) {
            ...
            if (loopCount != 1 && loopCount < path.length) {
                typeof aspectCount[aspect] == 'undefined' &&
        ...
        $searchInfo.find('.total-steps').text(stepCount);

        $aspectsList.after($searchInfo);
    ...
    createAspectsList: function () {
        ...
        this.aspects.forEach(function (aspect) {
            var $aspect = $(
                '<li class="aspect"><img><div class="aspect-text"><h4></h4><div class="desc"></div></div></li>'
            );
    ...
    initSelect2: function (element, data, value) {
        function format(d) {
            ...
            return (
                ...
                translate[aspect] +
                '.png" /><div class="aspect-name">' +
                translate[aspect] +
                '</div><div class="aspect-desc">' +
                aspect +
                '</div></div>'
            );
        }
    ...
    setVersions: function () {
        ...
        $.each(version_dictionary, function (key, version) {
            $('<option />').val(key).text(key).appendTo(self.c.$version);
        });
    ...
    watchForViewChanges: function () {
        ...
        $('#find_connection').on('click', function (e) {
            ...
            if (!self.c.$searchResults.children().length) {
                self.c.$search.removeClass(
                    'col-lg-offset-4 col-md-offset-4 col-sm-offset-3'
                );
        ...
        $('#swap').on('click', function () {
            var from = self.c.$from.val(),
                to = self.c.$to.val();
            self.c.$from.select2('val', to);
            self.c.$to.select2('val', from);
        });
        ...
        $('body').on(
            'mouseenter',
            '#avail .aspect, #search-results .aspect',
            function () {
